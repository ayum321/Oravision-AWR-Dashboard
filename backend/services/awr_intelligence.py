...
class AWRFinding:
    ...
    fix_detail: dict = field(default_factory=dict)  # structured fix payload
...
_WAIT_CATALOG: dict[str, dict] = {
    "db file sequential read": {
        ...
        "fix": (
            ...
            "4) INDEX SCAN STORM: if lat < 10ms AND io_pct > 40%: check clustering_factor.\n"
            "   SELECT index_name, blevel, clustering_factor, num_rows, leaf_blocks,\n"
            "          ROUND(clustering_factor/NULLIF(num_rows,0),4) cf_ratio\n"
            "   FROM dba_ind_statistics WHERE table_name = [hot_table]\n"
            "   ORDER BY clustering_factor DESC;\n"
            "   cf_ratio > 0.8 = CRITICAL (random I/O), > 0.3 = WARN (elevated I/O per scan)."
        ),
        "oracle_ref": "Oracle Perf Guide — I/O Configuration and Design",
        "tags": ["io", "index", "clustering_factor"],
        "hypothesis": "INDEX_SCAN_STORM",
        "hypothesis_trigger": "delta_pp > 5 AND culprit_sql.io_pct > 40 AND avg_wait_ms < 10",
    },
    ...
    "buffer busy waits": {
        "cat": "Contention", "lat_ms": 0, "pct_threshold": 3,
        "root": "Hot block contention — multiple sessions accessing same buffer cache block. Segment header (MSSM) or data block.",
        "fix": (
            "1) Identify hot block: SELECT p1 file#, p2 block# FROM v$session_wait WHERE event='buffer busy waits';\n"
            "2) Map to object: SELECT segment_name FROM dba_extents WHERE file_id=:file# AND :block# BETWEEN block_id AND block_id+blocks-1;\n"
            "3) Segment header → switch to ASSM. Data block → hash partition to distribute rows.\n"
            "4) Index leaf → increase INITRANS or consider reverse-key index."
        ),
        "oracle_ref": "Oracle Perf Guide — Buffer Cache Tuning",
        "tags": ["contention", "hot_block", "buffer_cache"],
    },
    "enq: TX - index contention": {
        "cat": "Locking", "lat_ms": 0, "pct_threshold": 2,
        "root": "Concurrent INSERTs on monotonically increasing index key — all sessions target same rightmost leaf block.",
        "fix": (
            "1) Identify hot index: SELECT current_obj# FROM v$active_session_history WHERE event LIKE 'enq: TX - index%';\n"
            "2) If monotonic key (sequence/timestamp) → ALTER INDEX REBUILD REVERSE;\n"
            "3) Alternatively: CREATE INDEX ... GLOBAL PARTITION BY HASH (key_col) PARTITIONS 8;\n"
            "4) For RAC: increase sequence CACHE."
        ),
        "oracle_ref": "Oracle Perf Guide — Index Contention",
        "tags": ["lock", "index", "contention"],
    },
    "resmgr:cpu quantum": {
        "cat": "Configuration", "lat_ms": 0, "pct_threshold": 1,
        "root": "Oracle Resource Manager throttling — sessions runnable but CPU-capped by consumer group limits.",
        "fix": (
            "1) Check active plan: SELECT value FROM v$parameter WHERE name='resource_manager_plan';\n"
            "2) Check consumer group limits: SELECT group_or_subplan, cpu_p1, max_active_sess_target_p1 FROM dba_rsrc_plan_directives;\n"
            "3) Increase CPU allocation or disable RM: ALTER SYSTEM SET resource_manager_plan='' SCOPE=BOTH;"
        ),
        "oracle_ref": "Oracle Perf Guide — Resource Manager",
        "tags": ["configuration", "cpu", "resource_manager"],
    },
    "read by other session": {
        "cat": "I/O", "lat_ms": 10, "pct_threshold": 3,
        "root": "Multiple sessions request same block from disk simultaneously. Session A reads; B,C,D wait for it to land in buffer cache.",
        "fix": (
            "1) Keep hot objects in KEEP buffer pool: ALTER TABLE ... STORAGE (BUFFER_POOL KEEP);\n"
            "2) Size db_keep_cache_size accordingly.\n"
            "3) Increase db_cache_size to reduce cache misses."
        ),
        "oracle_ref": "Oracle Perf Guide — Buffer Cache Tuning",
        "tags": ["io", "buffer_cache", "contention"],
    },
    "log buffer space": {
        "cat": "Redo", "lat_ms": 0, "pct_threshold": 1,
        "root": "Redo log buffer filling faster than LGWR can flush. DML rate exceeds log_buffer capacity.",
        "fix": (
            "1) Increase log_buffer (default may be too small for high-DML workloads).\n"
            "2) Check LGWR write speed: SELECT event, avg_wait_ms FROM v$system_event WHERE event='log file parallel write';\n"
            "3) Move redo logs to faster storage. Separate redo volumes from data."
        ),
        "oracle_ref": "Oracle Perf Guide — Redo Log Sizing",
        "tags": ["redo", "memory", "io"],
    },
...
_WAIT_DAG: dict[str, dict] = {
    ...
    "db file sequential read": {
        ...
        "root_causes": [
            ...
            "Index scan storm — clustering_factor ≈ num_rows causes ~1 physical read per row",
        ],
        "correlated": ["db file scattered read", "free buffer waits"],
        "dimension": "IO",
        "diagnostic_queries": {
            "SEGMENT_PHYSICAL_READS": "SELECT o.owner, o.object_name, o.object_type, SUM(s.physical_reads_delta) AS phys_reads, SUM(s.logical_reads_delta) AS logical_reads FROM dba_hist_seg_stat s JOIN dba_hist_seg_stat_obj o ON s.obj# = o.obj# AND s.dataobj# = o.dataobj# AND s.dbid = o.dbid WHERE s.snap_id BETWEEN :begin_snap AND :end_snap AND s.dbid = (SELECT dbid FROM v$database) GROUP BY o.owner, o.object_name, o.object_type ORDER BY phys_reads DESC FETCH FIRST 20 ROWS ONLY",
            "INDEX_HEALTH_CHECK": "SELECT index_name, blevel, clustering_factor, num_rows, leaf_blocks, ROUND(clustering_factor/NULLIF(num_rows,0),4) cf_ratio, ROUND(leaf_blocks/NULLIF(num_rows,0)*1000,2) leaves_per_1k_rows, last_analyzed, CASE WHEN last_analyzed < SYSDATE - 7 THEN 'STALE' ELSE 'OK' END AS stats_status FROM dba_ind_statistics WHERE table_name = :table_name AND owner = :schema ORDER BY clustering_factor DESC",
            "PLAN_INDEX_USAGE": "SELECT p.id, p.operation, p.options, p.object_owner, p.object_name, p.cost, p.cardinality, p.access_predicates, p.filter_predicates FROM dba_hist_sql_plan p WHERE p.sql_id = :sql_id AND p.dbid = (SELECT dbid FROM v$database) AND p.plan_hash_value = (SELECT plan_hash_value FROM dba_hist_sqlstat WHERE sql_id = :sql_id AND dbid = (SELECT dbid FROM v$database) ORDER BY elapsed_time DESC FETCH FIRST 1 ROW ONLY) ORDER BY p.id",
        },
    },
    "db file scattered read": {
    ...
    "buffer busy waits": {
        "parents": [],
        "root_causes": [
            "Hot block contention — concurrent DML on same block",
            "Segment header contention in MSSM tablespaces",
            "INITRANS too low on hot index or table",
        ],
        "correlated": ["latch: cache buffers chains", "enq: TX - index contention"],
        "dimension": "Contention",
    },
    "enq: TX - index contention": {
        "parents": [],
        "root_causes": [
            "Monotonic index key (sequence/timestamp) — all INSERTs target same rightmost leaf",
            "Hot right-hand leaf block serialisation",
        ],
        "correlated": ["buffer busy waits", "enq: TX - row lock contention"],
        "dimension": "Lock/Index",
    },
    "enq: HW - contention": {
        "parents": [],
        "root_causes": [
            "Concurrent INSERTs extending segment past HWM",
            "Segment not pre-allocated — AUTOALLOCATE giving small extents",
        ],
        "correlated": ["buffer busy waits"],
        "dimension": "Lock/Segment",
        "diagnostic_queries": {
            "SEGMENT_EXTENSION": "SELECT o.owner, o.object_name, o.object_type, SUM(s.space_allocated_delta) AS space_alloc_bytes FROM dba_hist_seg_stat s JOIN dba_hist_seg_stat_obj o ON s.obj# = o.obj# AND s.dataobj# = o.dataobj# AND s.dbid = o.dbid WHERE s.snap_id BETWEEN :begin_snap AND :end_snap AND s.dbid = (SELECT dbid FROM v$database) AND s.space_allocated_delta > 0 GROUP BY o.owner, o.object_name, o.object_type ORDER BY space_alloc_bytes DESC FETCH FIRST 10 ROWS ONLY",
            "INSERT_CULPRIT_SQL": "SELECT sql_id, executions, ROUND(elapsed_time/1e6/NULLIF(executions,0),3) avg_elapsed_s, rows_processed, module FROM dba_hist_sqlstat WHERE snap_id BETWEEN :begin_snap AND :end_snap AND dbid = (SELECT dbid FROM v$database) AND command_type = 2 ORDER BY elapsed_time DESC FETCH FIRST 20 ROWS ONLY",
        },
    },
    "resmgr:cpu quantum": {
        "parents": [],
        "root_causes": [
            "Resource Manager throttling — consumer group CPU limit reached",
            "Unintentional Resource Manager plan active",
        ],
        "correlated": [],
    ...
    "read by other session": {
        "parents": ["db file sequential read"],
        "root_causes": [
            "Same block requested from disk concurrently by multiple sessions",
            "Buffer cache undersized for concurrent workload",
        ],
        "correlated": ["db file sequential read", "free buffer waits"],
        "dimension": "IO/Contention",
    },
    "log buffer space": {
        "parents": ["log file sync"],
        "root_causes": [
            "log_buffer too small for DML rate",
            "LGWR storage write speed insufficient",
        ],
        "correlated": ["log file sync", "log file parallel write"],
        "dimension": "Redo/Memory",
    },
    "library cache lock": {
        "parents": ["latch: library cache"],
        "root_causes": [
            "DDL or recompile during peak execution hours",
            "Package invalidation by stats gather",
        ],
        "correlated": ["cursor: pin S wait on X", "latch: library cache"],
        "dimension": "Latch/DDL",
    },
}
...
# ══════════════════════════════════════════════════════════════════════════════
#  §10.5  STRUCTURED FIX STATEMENT GENERATOR
#         Produces an evidence-traceable, root-cause-specific fix payload.
#         Rules:
#           • Every fix must be traceable to evidence in the finding.
#           • Fixes are root-cause-specific, not symptom-only.
#           • Latch fixes are mapped to their subsystem — no generic advice.
#           • Unusual latch names → "targeted investigation required."
#           • ADDM used as corroboration only, never as the primary fix source.
#           • Conclusive root cause → direct action.
#           • Probable (not proven) root cause → investigation-first action.
# ══════════════════════════════════════════════════════════════════════════════

# Latch name → (subsystem, root_cause_type, fix_is_conclusive)
_LATCH_MAP: dict[str, tuple[str, str, bool]] = {
    "library cache":          ("Shared Pool / Parse",      "latch_contention", True),
    "shared pool":            ("Shared Pool / Memory",     "latch_contention", True),
    "cache buffers chains":   ("Buffer Cache / Hot Block", "latch_contention", True),
    "redo copy":              ("Redo / LGWR",              "commit_bottleneck", True),
    "redo allocation":        ("Redo / Log Buffer",        "commit_bottleneck", True),
    "row cache objects":      ("Dictionary Cache",         "latch_contention", False),
    "row cache lock":         ("Dictionary Cache",         "latch_contention", False),
    "enqueue hash chains":    ("Lock Management",          "locking",          False),
    "simulator lru latch":    ("Buffer Cache / Simulator", "latch_contention", False),
    "object queue header":    ("Buffer Cache / DBWR",      "latch_contention", False),
    "in memory undo latch":   ("IMU / Undo",               "latch_contention", False),
    "dml lock allocation":    ("DML / Lock Table",         "locking",          False),
    "session allocation":     ("Session Management",       "latch_contention", False),
    "active checkpoint queue":("Checkpoint / CKPT",        "latch_contention", False),
    "checkpoint queue latch": ("Checkpoint / CKPT",        "latch_contention", False),
    "file number translation":("File / I/O",               "io_bottleneck",    False),
}

_LATCH_FIX: dict[str, dict] = {
    "library cache": {
        "fix_statement": (
            "Eliminate hard parses by enforcing bind variables in application code. "
            "Every distinct literal SQL text competes for the library cache latch; "
            "cursor reuse removes that pressure entirely."
        ),
        "fix_reasoning": (
            "Library cache latch contention is caused by hard parses — each parse "
            "requires an exclusive latch hold to insert into the cursor cache. "
            "The evidence shows high library cache latch misses co-occurring with "
            "low soft-parse %, confirming parse pressure as the root cause."
        ),
        "action_1": "ALTER SYSTEM SET CURSOR_SHARING=FORCE SCOPE=BOTH; -- immediate workaround",
        "action_2": "ALTER SYSTEM SET SESSION_CACHED_CURSORS=200 SCOPE=SPFILE;",
        "action_3": (
            "Find literal-heavy SQL: SELECT sql_text, parse_calls, executions "
            "FROM v$sql WHERE parse_calls > executions * 0.9 "
            "ORDER BY parse_calls DESC FETCH FIRST 20 ROWS ONLY;"
        ),
        "validate_by": (
            "After change: Soft Parse % should exceed 95%. "
            "Library cache latch misses should drop below 0.5%."
        ),
    },
    "shared pool": {
        "fix_statement": (
            "Address shared pool pressure by sizing the pool per its advisory, "
            "pinning large packages, and reducing hard parses that fragment the pool."
        ),
        "fix_reasoning": (
            "Shared pool latch misses indicate either pool fragmentation (frequent "
            "allocation/deallocation from hard parses) or pool undersizing. "
            "The fix must address the root: reduce churn (bind variables) AND "
            "ensure adequate pool size per V$SHARED_POOL_ADVICE."
        ),
        "action_1": (
            "SELECT shared_pool_size_for_estimate, estd_lc_size, estd_lc_time_saved_factor "
            "FROM v$shared_pool_advice; -- size to ≥1.0 time-saved-factor"
        ),
        "action_2": "EXEC DBMS_SHARED_POOL.KEEP('SCHEMA.LARGE_PACKAGE','P');",
        "action_3": "Enforce bind variables; CURSOR_SHARING=FORCE as interim measure.",
        "validate_by": "Shared pool latch miss rate < 0.5%; Library Hit % > 99%.",
    },
    "cache buffers chains": {
        "fix_statement": (
            "Identify and eliminate the hot block. The latch protects the hash bucket "
            "containing the hot buffer — the fix is to remove the heat, not the latch."
        ),
        "fix_reasoning": (
            "Cache buffers chains latch contention is hot-block syndrome: many sessions "
            "simultaneously requesting the same 8 KB buffer. Common causes are a "
            "hot index leaf block (sequence-driven inserts), a hot segment header, "
            "or an unpartitioned table with a single DML hot spot."
        ),
        "action_1": (
            "SELECT obj, dbarfil, dbablk, tch FROM x$bh "
            "WHERE tch > 100 ORDER BY tch DESC FETCH FIRST 10 ROWS ONLY; "
        ...
        "action_2": (
            "If index leaf block: use reverse-key index or hash-partition the index. "
            "If sequence: ALTER SEQUENCE s CACHE 1000 ORDER; "
            "If segment header: ASSM tablespace removes freelist contention."
        ),
        "action_3": (
            "Confirm with ASH: SELECT p1, p2, p3, COUNT(*) FROM v$active_session_history "
            "WHERE event = 'latch: cache buffers chains' AND sample_time > SYSDATE-1/24 "
            "GROUP BY p1, p2, p3 ORDER BY 4 DESC;"
        ),
        "validate_by": (
            "Cache buffers chains latch miss rate drops below 1%. "
            "Hot block touch count (x$bh.tch) redistributes."
        ),
    },
    "redo copy": {
        "fix_statement": (
            "Redo copy latch contention means LGWR is bottlenecked writing redo buffers. "
            "Move redo logs to fastest available storage; enable async I/O."
        ),
        "fix_reasoning": (
            "The redo copy latch is held while copying redo change vectors into the redo buffer. "
            "High misses indicate the buffer is full because LGWR cannot flush to disk fast enough — "
            "the fix is to accelerate LGWR I/O, not to tune the latch directly."
        ),
        "action_1": "Move online redo logs to NVMe or RAID10 storage (never RAID5).",
        "action_2": "Verify DISK_ASYNCH_IO=TRUE; set LOG_ARCHIVE_DEST to separate device.",
        "action_3": (
            "Check LGWR trace: SELECT event, time_waited FROM v$session "
            "WHERE program LIKE '%LGWR%';"
        ),
        "validate_by": "Log file sync avg wait < 3ms; redo copy latch miss rate < 0.5%.",
    },
    "redo allocation": {
        "fix_statement": (
            "Redo allocation latch contention: log buffer too small or commit rate too high. "
            "Increase LOG_BUFFER and reduce commit frequency."
        ),
        "fix_reasoning": (
            "The redo allocation latch is acquired to allocate space in the log buffer for "
            "each change vector. High contention means the buffer is insufficient for the "
            "commit rate, or LGWR is too slow to free space."
        ),
        "action_1": "ALTER SYSTEM SET LOG_BUFFER=134217728; -- 128MB",
        "action_2": "Batch commits in bulk DML: reduce per-row commits to per-batch.",
        "action_3": "Check LGWR I/O latency: SELECT * FROM v$filestat WHERE file# IN (SELECT member FROM v$logfile);",
        "validate_by": "Redo NoWait % > 99.9%; redo allocation latch miss rate < 0.5%.",
    },
}


def _resolve_latch_subtype(title: str, finding_id: str) -> tuple[str, str, bool, dict]:
    """
    Map a latch finding to its subsystem. Returns:
    (subsystem_name, root_cause_type, is_conclusive, fix_dict).
    If the latch cannot be identified, returns investigation-first advice.
    """
    combined = (title + " " + finding_id).lower()
    for key, (subsystem, rct, conclusive) in _LATCH_MAP.items():
        if key in combined:
            fix = _LATCH_FIX.get(key)
            if fix:
                return subsystem, rct, conclusive, fix
            # Mapped but no detailed fix (weak subsystem)
            return subsystem, rct, False, {
                "fix_statement": (
                    f"Investigate the {subsystem} subsystem. "
                    f"The latch '{key}' protects {subsystem} structures — "
                    f"targeted subsystem investigation required."
                ),
                "fix_reasoning": (
                    f"This latch is mapped to {subsystem} but the specific contention "
                    f"pattern requires ASH investigation to confirm the exact hot path."
                ),
                "action_1": (
                    f"SELECT event, p1text, p1, p2text, p2, COUNT(*) cnt "
                    f"FROM v$active_session_history "
                    f"WHERE event LIKE '%latch%' AND sample_time > SYSDATE-1/24 "
                    f"GROUP BY event, p1text, p1, p2text, p2 ORDER BY cnt DESC;"
                ),
                "action_2": "Review AWR Latch Activity section for sleep counts and spin gets.",
                "action_3": "Engage Oracle Support with AWR + ASH report for targeted RCA.",
                "validate_by": f"Latch miss rate for '{key}' below 1%.",
            }
    # Unknown latch
    return "Unknown", "latch_contention", False, {
        "fix_statement": (
            "Targeted subsystem investigation required. This latch name is not in the "
            "standard catalog — generic advice (SHARED_POOL_SIZE, bind variables) "
            "may not apply. Profile the specific latch to identify its protection scope."
        ),
        "fix_reasoning": (
            "An unknown or uncommon latch name cannot be reliably mapped to a fix without "
            "confirming what Oracle internal structure it protects. "
            "Applying standard latch fixes without evidence risks masking the real issue."
        ),
        "action_1": (
            "SELECT child#, gets, misses, sleeps, spin_gets "
            "FROM v$latch_children "
            "WHERE name = '<latch_name>' ORDER BY misses DESC;"
        ),
        "action_2": (
            "Cross-reference with ASH: "
            "SELECT p1, p1raw, COUNT(*) FROM v$active_session_history "
            "WHERE event LIKE '%latch%' AND sample_time > SYSDATE-1/24 "
            "GROUP BY p1, p1raw ORDER BY 3 DESC;"
        ),
        "action_3": "Raise SR with Oracle Support; provide AWR + latch statistics sections.",
        "validate_by": "Latch miss rate drops below 1% after targeted fix.",
    }


def _generate_fix_statement(
    f: AWRFinding,
    good_data: dict | None = None,
    bad_data: dict | None = None,
) -> dict:
    """
    Generate a structured, evidence-traceable fix payload for a finding.
    Returns dict with keys: title, severity, confidence, root_cause_type,
    fix_statement, fix_reasoning, action_1, action_2, action_3, validate_by, notes.
    """
    tags = set(f.tags or [])
    fid  = f.id.lower()
    title_l = f.title.lower()
    is_new     = "[new]" in title_l
    is_worse   = "[worse]" in title_l
    is_sus     = "[sustained]" in title_l
    sql_context = f", SQL_ID(s): {', '.join(f.sql_ids)}" if f.sql_ids else ""
    addm_confirmed = "addm" in tags or any("addm" in ev.lower() for ev in (f.evidence or []))

    # ── Step 1: Determine root_cause_type ────────────────────────────────────
    if "lock" in tags and "blocking" in tags:
        rct = "locking"
    elif "latch" in tags or "latch" in fid:
        rct = "latch_contention"
    elif {"parse", "cursor"} & tags and "latch" not in tags:
        rct = "latch_contention" if "library" in fid else "cpu_pressure"
    elif "redo" in tags or "commit" in tags:
        rct = "commit_bottleneck"
    elif "io" in tags and "sql" not in tags:
        rct = "io_bottleneck"
    elif "cpu" in tags and "sql" not in tags:
        rct = "cpu_pressure"
    elif "sql" in tags and (is_new or "sql_id" in fid):
        rct = "new_sql"
    elif "sql" in tags and (is_worse or is_sus or "regress" in fid or "cpu" in fid or "gets" in fid):
        rct = "sql_regression"
    elif "pga" in tags or "temp" in tags:
        rct = "io_bottleneck"
    elif "rac" in tags:
        rct = "mixed"
    elif is_new or is_worse:
        rct = "workload_growth" if "workload" in fid or f.category in ("CPU", "I/O") else "mixed"
    else:
        rct = "inconclusive"

    # ── Step 2: Determine if finding is conclusive or probable ───────────────
    conclusive = f.confidence == "HIGH" or (addm_confirmed and f.confidence != "LOW")

    # ── Step 3: Route to the right fix generator ─────────────────────────────

    # --- LATCH ---
    if rct == "latch_contention" or "latch" in tags:
        subsystem, rct_resolved, latch_conclusive, lf = _resolve_latch_subtype(f.title, f.id)
        conclusive = conclusive and latch_conclusive
        change_note = ""
        if is_new:
            change_note = f"This latch contention is NEW — not present in the good period. Workload change or schema change likely introduced it."
        elif is_worse:
            change_note = f"This latch contention WORSENED versus the good period — ruling out random fluctuation."
        addm_note = "ADDM corroborates this finding." if addm_confirmed else ""
        return {
            "title": f.title.lstrip("[NEWORSUDAI] "),
            "severity": f.severity,
            "confidence": f.confidence,
            "root_cause_type": rct_resolved,
            "fix_statement": lf["fix_statement"],
            "fix_reasoning": lf["fix_reasoning"],
            "action_1": lf["action_1"],
            "action_2": lf["action_2"],
            "action_3": lf["action_3"],
            "validate_by": lf["validate_by"],
            "notes": " | ".join(filter(None, [
                f"Latch subsystem: {subsystem}.",
                change_note,
                addm_note,
            ])),
        }
    ...
    # --- SQL REGRESSION ---
    if rct == "sql_regression":
        sql_ref = f.sql_ids[0] if f.sql_ids else "top SQL"
        gets_ev = next((e for e in (f.evidence or []) if "gets" in e.lower()), "")
        plan_ev = any("plan" in e.lower() or "hash" in e.lower() for e in (f.evidence or []))
        if plan_ev:
            fs = (
                f"SQL {sql_ref} has a regressed execution plan{sql_context}. "
                f"Pin the previous plan via SQL Plan Baseline or use a SQL Profile to stabilise."
            )
            fr = (
                f"The evidence shows SQL {sql_ref} elapsed time increased with a plan hash change — "
                f"classic plan regression. The optimizer chose a suboptimal plan, likely due to "
                f"stale statistics, a histogram mismatch, or a new optimizer feature."
            )
            a1 = f"SELECT * FROM TABLE(DBMS_XPLAN.DISPLAY_CURSOR('{sql_ref}',null,'ALLSTATS LAST +PEEKED_BINDS'));"
            a2 = (
                f"Pin last good plan: EXEC DBMS_SPM.LOAD_PLANS_FROM_CURSOR_CACHE("
                f"sql_id=>'{sql_ref}', plan_hash_value=><good_hash>);"
            )
            a3 = f"EXEC DBMS_STATS.GATHER_TABLE_STATS('SCHEMA','TABLE',cascade=>TRUE,method_opt=>'FOR ALL COLUMNS SIZE AUTO');"
            vby = f"SQL {sql_ref} elapsed/exec returns to baseline; plan hash stabilises."
        else:
            fs = (
                f"SQL {sql_ref} has regressed{sql_context}. "
                f"Gather fresh optimizer statistics and inspect the execution plan for "
                f"cardinality misestimates driving suboptimal access paths."
            )
            fr = (
                f"SQL {sql_ref} elapsed time increased significantly versus the good period "
                f"without a plan hash change — likely caused by stale statistics causing "
                f"worse row estimates, or a data volume change changing the optimal access path."
            )
            a1 = f"SELECT * FROM TABLE(DBMS_XPLAN.DISPLAY_CURSOR('{sql_ref}',null,'ALLSTATS LAST +PEEKED_BINDS'));"
            a2 = f"EXEC DBMS_STATS.GATHER_TABLE_STATS('SCHEMA','TABLE',cascade=>TRUE); -- refresh stats"
            a3 = (
                f"Run SQL Tuning Advisor: DECLARE t VARCHAR2(100); "
                f"BEGIN t:=DBMS_SQLTUNE.CREATE_TUNING_TASK(sql_id=>'{sql_ref}'); "
                f"DBMS_SQLTUNE.EXECUTE_TUNING_TASK(t); "
                f"DBMS_OUTPUT.PUT_LINE(DBMS_SQLTUNE.REPORT_TUNING_TASK(t)); END;"
            )
            vby = f"SQL {sql_ref} elapsed/exec within 20% of baseline after stats refresh."
        return {
            "title": f"SQL Regression: {sql_ref}",
            "severity": f.severity,
            "confidence": f.confidence,
            "root_cause_type": "sql_regression",
            "fix_statement": fs,
            "fix_reasoning": fr,
            "action_1": a1,
            "action_2": a2,
            ...
            "validate_by": vby,
            "notes": (
                f"{gets_ev} | " if gets_ev else ""
            ) + ("ADDM corroborates." if addm_confirmed else ""),
        }

    # --- NEW SQL ---
    if rct == "new_sql":
        sql_ref = f.sql_ids[0] if f.sql_ids else "new SQL"
        return {
            "title": f"New SQL Offender: {sql_ref}",
            "severity": f.severity,
            "confidence": f.confidence,
            "root_cause_type": "new_sql",
            "fix_statement": (
                f"SQL {sql_ref} is new to this period and immediately a top consumer{sql_context}. "
                f"Inspect the execution plan, verify optimizer statistics are current, "
                f"and confirm whether this SQL is expected from application changes."
            ),
            "fix_reasoning": (
                f"A new SQL not seen in the good period accounts for significant DB time. "
                f"New SQLs with large impact either have no statistics, a bad initial plan, "
                f"or represent unintended code paths (e.g., missing bind variable, new feature)."
            ),
            "action_1": f"SELECT * FROM TABLE(DBMS_XPLAN.DISPLAY_CURSOR('{sql_ref}',null,'ALLSTATS LAST +PEEKED_BINDS'));",
            "action_2": f"SELECT module, action, parsing_schema_name FROM v$sql WHERE sql_id='{sql_ref}';",
            "action_3": (
                f"EXEC DBMS_STATS.GATHER_TABLE_STATS for tables accessed by this SQL; "
                f"confirm bind variable usage (SELECT force_matching_signature FROM v$sql WHERE sql_id='{sql_ref}')."
            ),
            "validate_by": f"SQL {sql_ref} elapsed/exec within acceptable range; plan is stable.",
            "notes": "Appeared in problem period only — absent from baseline. Application deploy or data volume change likely." + (
                " ADDM corroborates." if addm_confirmed else ""
            ),
        }

    # --- IO BOTTLENECK ---
    if rct == "io_bottleneck":
        temp_spill = "pga" in tags or "temp" in tags
        if temp_spill:
            return {
                "title": "PGA Spill to Temp",
                "severity": f.severity,
                "confidence": f.confidence,
                "root_cause_type": "io_bottleneck",
                "fix_statement": (
                    "PGA_AGGREGATE_TARGET is too small for the sort/hash join workload. "
                    "Increase it per V$PGA_TARGET_ADVICE to eliminate temp spills."
                ),
                "fix_reasoning": (
                    "Direct path read/write temp waits occur when a sort or hash join "
                    "cannot complete in PGA memory and spills to the temp tablespace. "
                    "This is 100× slower than in-memory operation. The evidence shows "
                    "direct path temp waits as a significant DB time consumer."
                ),
                "action_1": (
                ...
                "action_3": "ALTER SYSTEM SET PGA_AGGREGATE_TARGET=<advisory_value>;",
                "validate_by": "In-memory Sort % > 99%; direct path temp waits < 1% of DB time.",
                "notes": ("ADDM corroborates." if addm_confirmed else ""),
            }
        # Physical I/O
        lat_ev = next((e for e in (f.evidence or []) if "threshold" in e.lower() or "ms" in e.lower()), "")
        return {
            "title": "Physical I/O Bottleneck",
            "severity": f.severity,
            "confidence": f.confidence if conclusive else "MEDIUM",
            "root_cause_type": "io_bottleneck",
            "fix_statement": (
                "Storage latency is the primary bottleneck. "
                + ("Latency exceeds acceptable thresholds for this workload — " if lat_ev else "")
                + "resolve at storage tier first, then optimise SQL access patterns."
            ),
            "fix_reasoning": (
                f"The evidence shows I/O waits consuming significant DB time{sql_context}. "
                f"If latency > 20ms for single-block reads, the storage subsystem is saturated. "
                f"If latency is acceptable but volume is high, the SQL access pattern needs tuning."
            ),
            "action_1": (
                "SELECT event, time_waited, average_wait, total_waits "
                "FROM v$system_event WHERE event LIKE 'db file%' ORDER BY time_waited DESC;"
            ),
            "action_2": (
                "Find top physical-read SQL: "
                "SELECT sql_id, disk_reads, buffer_gets FROM v$sql "
                "ORDER BY disk_reads DESC FETCH FIRST 10 ROWS ONLY;"
            ),
            "action_3": "Check storage: SAN alerts, ASM disk I/O (V$ASM_DISK_STAT), OS iostat.",
            "validate_by": "Single-block read avg < 5ms (SSD) or < 15ms (HDD); I/O wait < 10% DB time.",
            "notes": (lat_ev or "") + (" | ADDM corroborates." if addm_confirmed else ""),
        }

    # --- COMMIT / REDO BOTTLENECK ---
    if rct == "commit_bottleneck":
        return {
            "title": "Commit / Redo Bottleneck",
            "severity": f.severity,
            "confidence": f.confidence,
            "root_cause_type": "commit_bottleneck",
            "fix_statement": (
                "LGWR is too slow flushing redo to disk — every COMMIT incurs this latency. "
                "Move redo logs to dedicated fastest-available storage and reduce commit frequency."
            ),
            "fix_reasoning": (
                "Log file sync latency directly equals user COMMIT response time. "
                "High log file sync wait means LGWR I/O is the rate limiter. "
                "Every user committing waits for this. "
                "The fix must be at the storage tier (LGWR I/O speed) and/or "
                "at the application tier (fewer, batched commits)."
            ),
            "action_1": (
                "Confirm LGWR I/O: SELECT event, total_waits, time_waited, average_wait "
                "FROM v$system_event WHERE event = 'log file parallel write';"
            ),
            "action_2": "Move redo logs to NVMe / dedicated RAID10 storage; never RAID5.",
            "action_3": "Batch application commits; verify DISK_ASYNCH_IO=TRUE; consider NOLOGGING for bulk loads.",
            "validate_by": "Log file sync avg wait < 3ms; log file parallel write avg < 1ms.",
            "notes": ("ADDM corroborates." if addm_confirmed else ""),
        }
    ...
    if rct == "cpu_pressure":
        return {
            "title": "CPU Pressure / Hard Parse",
            "severity": f.severity,
            "confidence": f.confidence,
            "root_cause_type": "cpu_pressure",
            "fix_statement": (
                "CPU is the bottleneck. "
                + (
                    "Eliminate hard parses — they consume disproportionate CPU per call "
                    "and serialise on the library cache latch. "
                    if "parse" in tags else
                    "Profile ON-CPU sessions with ASH to identify top SQL offenders. "
                )
                + "SQL tuning is the primary lever."
            ),
            "fix_reasoning": (
                f"Evidence shows CPU saturation or high hard-parse CPU consumption{sql_context}. "
                + (
                    "Hard parses each require full plan generation — a CPU-intensive, serialised operation."
                    if "parse" in tags else
                    "Sessions queue for CPU — the primary fix is to reduce per-execution work."
                )
            ),
            "action_1": (
                "SELECT sql_id, COUNT(*) cnt FROM v$active_session_history "
            ...
            "action_3": (
                "If parse-driven: ALTER SYSTEM SET CURSOR_SHARING=FORCE SCOPE=BOTH; "
                "If general CPU: run SQL Tuning Advisor on top CPU SQL."
            ),
            "validate_by": "AAS / CPU count < 0.8; Soft Parse % > 95%.",
            "notes": ("ADDM corroborates." if addm_confirmed else ""),
        }
    ...
    if rct == "locking":
        return {
            "title": "Row / Table Lock Contention",
            "severity": f.severity,
            "confidence": f.confidence,
            "root_cause_type": "locking",
            "fix_statement": (
                "Active blocking sessions are serialising concurrent DML. "
                "Find and eliminate the root transaction holding the lock — "
                "do not tune the symptom (waiter), fix the holder."
            ),
            "fix_reasoning": (
                "TX/TM enqueue waits indicate sessions waiting on a lock held by another transaction. "
                "Root cause is either long-running transactions, missing FK indexes (TM), "
                "or application design requiring uncommitted row access across sessions."
            ),
            "action_1": (
                "SELECT blocking_session, sid, serial#, wait_class, sql_id, event "
                "FROM v$session WHERE blocking_session IS NOT NULL AND wait_class='Application';"
            ),
            "action_2": (
                "For TM lock: check missing FK indexes — "
                "SELECT c.table_name, c.constraint_name FROM dba_constraints c "
                "WHERE c.constraint_type='R' AND NOT EXISTS ("
                "SELECT 1 FROM dba_indexes i WHERE i.table_name=c.table_name AND "
                "i.index_name IN (SELECT index_name FROM dba_ind_columns WHERE "
                "column_name=(SELECT column_name FROM dba_cons_columns WHERE "
                "constraint_name=c.constraint_name AND rownum=1)));"
            ),
            "action_3": "Commit sooner in blocker transactions; partition hot-row tables.",
            "validate_by": "Enqueue wait time < 1% of DB time; no blocking_session rows in V$SESSION.",
            "notes": ("ADDM corroborates." if addm_confirmed else ""),
        }

    # --- WORKLOAD GROWTH ---
    if rct == "workload_growth":
        return {
            "title": "Workload Volume Growth",
            "severity": f.severity,
            "confidence": "MEDIUM",
            "root_cause_type": "workload_growth",
            "fix_statement": (
                "The workload volume has increased versus the baseline period — "
                "this is a capacity issue, not a tuning issue. "
                "Scale hardware or reduce workload to restore headroom."
            ),
            "fix_reasoning": (
                "The evidence shows metrics uniformly worse versus the good period "
                "without a change in wait event composition. This is consistent with "
                "more sessions doing more work — the system is not broken, it is saturated."
            ),
            "action_1": "Compare AAS, executes/sec, and logons/sec between periods to confirm volume growth.",
            "action_2": "Review application-level batch schedules and report jobs — stagger peak load.",
            "action_3": "Evaluate CPU/memory capacity upgrade or workload management (Oracle Resource Manager).",
            "validate_by": "AAS / CPU count < 0.8 after capacity change or workload reduction.",
            "notes": ("ADDM corroborates." if addm_confirmed else "Probable workload growth — confirm with load profile deltas."),
        }
    ...
    return {
        ...
        "fix_statement": (
            "Multiple signals present — conclusive root cause not yet established. "
            "Investigate the primary wait event with ASH to confirm the bottleneck "
            "before committing to a fix path."
        ),
        "fix_reasoning": (
            "The finding evidence does not map cleanly to a single root-cause category. "
            "Either multiple causes are active simultaneously, or the AWR data does not "
            "contain sufficient detail to confirm directionality."
        ),
        "action_1": (
            f"SELECT event, sql_id, module, COUNT(*) cnt "
            f"FROM v$active_session_history "
            f"WHERE sample_time > SYSDATE - 1/24 "
            f"GROUP BY event, sql_id, module ORDER BY cnt DESC FETCH FIRST 20 ROWS ONLY;"
        ),
        "action_2": "Review AWR Top Events and Top SQL sections for corroborating patterns.",
        "action_3": "Engage Oracle Support with full AWR + ASH report if pattern persists.",
        "validate_by": "Root cause confirmed via ASH; targeted fix applied; DB time < baseline.",
        "notes": ("ADDM corroborates." if addm_confirmed else ""),
    }
...
def run_intelligence(
    ...
    # 7. Structured fix statements (evidence-traceable, root-cause-specific)
    for f in ranked:
        f.fix_detail = _generate_fix_statement(f)
...
def run_intelligence_compare(
    ...
    # Structured fix statements — pass both periods so context is available
    for f in ranked:
        f.fix_detail = _generate_fix_statement(f, good_data, bad_data)
