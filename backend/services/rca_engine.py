...
PATHOLOGY_MAP: dict[str, dict] = {
    ...
    "free buffer waits": {
        ...
        "causal_children": ["buffer busy waits", "enq: fb - contention", "db file sequential read",
                             "enq: ko - fast object checkpoint", "enq: cr - block range reuse ckpt"],
    ...
    "enq: ko - fast object checkpoint": {
        "category": "configuration",
        "meaning": "Oracle is forcing an object-level checkpoint — flushing every dirty buffer for a specific segment to disk before an operation that needs a clean, consistent view of it (e.g. a large parallel DML, direct-path load, or DDL). Sessions block here while DBWR drains the backlog. This is a downstream symptom of the same dirty-buffer pressure that produces free buffer waits — it is DBWR being unable to keep up, made visible as an explicit enqueue instead of a generic buffer wait.",
        "investigate": ["Which SQL/segment triggered the checkpoint (Segment Statistics — DB Block Changes / Physical Writes)",
                        "PARALLEL degree of the driving DML vs CPU count (over-parallelized DML dirties buffers faster than DBWR can flush)",
                        "db file parallel write avg wait (confirms DBWR is the bottleneck, not the enqueue itself)",
                        "Whether the driving statement is a single large transaction vs batched commits"],
        "causal_parents": ["free buffer waits"],
        "causal_children": [],
        "fix": "Reduce PARALLEL degree on the driving DML to at or below CPU count. Batch large MERGE/INSERT/UPDATE/DELETE operations into smaller committed chunks instead of one giant transaction. Increase DB_WRITER_PROCESSES / enable async I/O so DBWR can drain dirty buffers faster.",
    },
    "enq: cr - block range reuse ckpt": {
        "category": "configuration",
        "meaning": "A session needs to reuse a block range (e.g. during high-water-mark extension or space reclamation for a large DML) and must wait for a checkpoint to guarantee the blocks being reused are safely on disk first. Appears alongside enq: KO — fast object checkpoint as the second half of the same DBWR/checkpoint-storm mechanism: both are the price of forcing dirty buffers to disk before Oracle can safely proceed.",
        "investigate": ["Same driving SQL/segment as any concurrent enq: KO — fast object checkpoint wait",
                        "DB Block Changes / Physical Writes on the target segment (Segment Statistics)",
                        "PARALLEL degree vs CPU count for the driving DML",
                        "Note: this event is reported in the AWR Background Wait Events section, not Foreground — it will not appear in the Top 10 Foreground Events table even when it materially contributed to the stall"],
        "causal_parents": ["free buffer waits"],
        "causal_children": [],
        "fix": "Same remediation family as enq: KO — fast object checkpoint: reduce PARALLEL degree, batch the DML into smaller commits, increase DBWR throughput.",
    },
    "enq: fb - contention": {
        "category": "configuration",
