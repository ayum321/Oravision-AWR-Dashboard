...
# Find the start: the line containing "Connecting Dots Chain"
marker_text = 'Connecting Dots Chain'
marker_pos = content.index(marker_text)
# Go back to start of the line
line_start = content.rfind('\n', 0, marker_pos) + 1

# Find the IIFE closing: "            })()}" after the chain section
# We need the })() that's the closing of this specific IIFE
end_search = marker_pos
# The pattern is "            })()}" — the IIFE that wraps the entire SRE card
iife_close = "            })()}"
end_pos = content.index(iife_close, end_search)
end_pos += len(iife_close)

old_block = content[line_start:end_pos]
print(f"Old block: {len(old_block)} chars, starts at char {line_start}")
...
new_block = """                <!-- Connecting Dots Chain (evidence-based from ctx.connectingDots) -->
                ...
                <div style="background:rgba(10,16,32,0.8);border:1px solid ${ctx.connectingDots.color}25;border-radius:10px;padding:14px 16px">
                    <div class="flex items-center justify-between gap-2 mb-3">
                        <div class="flex items-center gap-2">
                            ...
                            <div class="text-[10px] font-bold uppercase tracking-wider" style="color:${ctx.connectingDots.color}">Connecting Dots """ + chr(8212) + """ ${esc(ctx.connectingDots.title)}</div>
                        </div>
                    ...
                    <div class="flex items-start gap-0 flex-wrap" style="row-gap:8px">
                        ${ctx.connectingDots.chain.map((node,ni) =>
                            '<div style="background:'+node.bg+';border:1px solid '+node.col+'30;border-radius:8px;padding:8px 12px;min-width:130px;max-width:180px">'+
                                '<div style="font-size:9px;color:'+node.col+';font-weight:900;text-transform:uppercase;letter-spacing:0.4px;margin-bottom:3px">'+(node.risk?""" + repr(chr(9888)) + """[0]+' RISK: ':'')+esc(node.label)+'</div>'+
                                '<div style="font-size:9px;color:#94a3b8;line-height:1.4">'+esc(node.sub)+'</div>'+
                            '</div>'+
                            (ni<ctx.connectingDots.chain.length-1?'<div style="padding:0 4px;color:#334155;font-size:18px;font-weight:bold;align-self:center;flex-shrink:0">""" + chr(8594) + """</div>':'')
                        ).join('')}
                    </div>
                ...
                <div style="background:rgba(16,185,129,0.06);border:1px solid rgba(16,185,129,0.15);border-radius:10px;padding:14px 16px">
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        <div class="text-[10px] font-bold text-green-400 uppercase tracking-wider">System Healthy """ + chr(8212) + """ No Causal Chain Detected</div>
                    </div>
                ...
                <div style="background:rgba(10,16,32,0.7);border:1px solid #1e293b;border-radius:10px;padding:12px 16px;margin-top:8px">
                    ...
                    ${ctx.addmCorroboration.matches.map(m => '<div style="font-size:9px;color:#6ee7b7;padding:2px 0">""" + chr(10003) + """ '+esc(m.finding||m.description||m.name||'ADDM Finding')+'</div>').join('')}
                    ` : `
...
content = content[:line_start] + new_block + content[end_pos:]
